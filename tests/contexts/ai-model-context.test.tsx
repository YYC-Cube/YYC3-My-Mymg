...
describe('AIModel Context', () => {
  ...
  it('should remove an AI model', () => {
    const { result } = renderHook(() => useAIModel(), { wrapper })
    act(() => {
      result.current.addAIModel({ name: 'Test', provider: 'ollama', endpoint: 'http://localhost', apiKey: '', isActive: false })
    })
  ...
  it('should update an AI model', () => {
    const { result } = renderHook(() => useAIModel(), { wrapper })
    act(() => {
      result.current.addAIModel({ name: 'Old', provider: 'custom', endpoint: 'http://old', apiKey: '', isActive: false })
    })
    const id = result.current.aiModels[0].id
  ...
  it('should activate an AI model', () => {
    const { result } = renderHook(() => useAIModel(), { wrapper })
    act(() => {
      result.current.addAIModel({ name: 'M1', provider: 'openai', endpoint: 'http://a', apiKey: 'k1', isActive: false })
      result.current.addAIModel({ name: 'M2', provider: 'ollama', endpoint: 'http://b', apiKey: '', isActive: false })
    })
  ...
  it('should clear active model id when removed', () => {
    const { result } = renderHook(() => useAIModel(), { wrapper })
    act(() => {
      result.current.addAIModel({ name: 'Active', provider: 'openai', endpoint: 'http://a', apiKey: 'k1', isActive: false })
    })
    const id = result.current.aiModels[0].id
  ...
  it('should persist models to localStorage', () => {
    const { result } = renderHook(() => useAIModel(), { wrapper })
    act(() => {
      result.current.addAIModel({ name: 'Persist', provider: 'custom', endpoint: 'http://test', apiKey: 'key', isActive: false })
    })
  ...
  it('should persist active model id to localStorage', () => {
    const { result } = renderHook(() => useAIModel(), { wrapper })
    act(() => {
      result.current.addAIModel({ name: 'X', provider: 'openai', endpoint: 'http://a', apiKey: 'k', isActive: false })
    })
    const id = result.current.aiModels[0].id
    act(() => result.current.activateAIModel(id))
