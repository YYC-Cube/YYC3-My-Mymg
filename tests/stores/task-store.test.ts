...
describe('TaskStore — Types & Constants', () => {
  ...
  it('should have valid task structure', () => {
    for (const task of INITIAL_TASKS) {
      ...
      expect(['todo', 'in-progress', 'review', 'done', 'blocked']).toContain(task.status)
      expect(['critical', 'high', 'medium', 'low']).toContain(task.priority)
      expect(['feature', 'bug', 'refactor', 'test', 'documentation', 'other']).toContain(task.type)
  ...
  it('should have valid reminder structure', () => {
    for (const r of INITIAL_REMINDERS) {
      ...
      expect(['deadline', 'dependency', 'blocking', 'progress', 'custom']).toContain(r.type)
  ...
  it('should have tasks with valid subtask structure', () => {
    const withSubs = INITIAL_TASKS.filter((t) => t.subtasks && t.subtasks.length > 0)
...
describe('TaskStore — Store Actions', () => {
  ...
  it('should update a task', () => {
    useTaskStore.getState().addTask({ title: 'Original', description: '', status: 'todo', priority: 'low', type: 'bug', source: 'manual' })
    const task = useTaskStore.getState().tasks[0]
    useTaskStore.getState().updateTask(task.id, { title: 'Updated', priority: 'critical' })
    const updated = useTaskStore.getState().tasks.find((t: Task) => t.id === task.id)!
    expect(updated.title).toBe('Updated')
    expect(updated.priority).toBe('critical')
  })

  it('should delete a task', () => {
    const id = useTaskStore.getState().addTask({ title: 'ToDelete', description: '', status: 'todo', priority: 'low', type: 'bug', source: 'manual' })
    useTaskStore.getState().deleteTask(id)
    expect(useTaskStore.getState().tasks.find((t: Task) => t.id === id)).toBeUndefined()
  })

  it('should archive a task', () => {
    const id = useTaskStore.getState().addTask({ title: 'Archive', description: '', status: 'todo', priority: 'low', type: 'bug', source: 'manual' })
  ...
  it('should duplicate a task', () => {
    useTaskStore.getState().addTask({ title: 'Original', description: 'desc', status: 'todo', priority: 'medium', type: 'feature', source: 'manual' })
  ...
  it('should move a task to new status', () => {
    const id = useTaskStore.getState().addTask({ title: 'Move', description: '', status: 'todo', priority: 'medium', type: 'feature', source: 'manual' })
  ...
  it('should add and toggle a subtask', () => {
    const id = useTaskStore.getState().addTask({ title: 'Sub', description: '', status: 'todo', priority: 'medium', type: 'feature', source: 'manual' })
    ...
    const toggled = useTaskStore.getState().tasks.find((t: Task) => t.id === id)!
    expect(toggled.subtasks![0].isCompleted).toBe(true)
  })

  it('should delete a subtask', () => {
    const id = useTaskStore.getState().addTask({ title: 'DelSub', description: '', status: 'todo', priority: 'medium', type: 'feature', source: 'manual' })
    useTaskStore.getState().addSubtask(id, 'Sub 1')
    const subId = useTaskStore.getState().tasks.find((t: Task) => t.id === id)!.subtasks![0].id
    useTaskStore.getState().deleteSubtask(id, subId)
    expect(useTaskStore.getState().tasks.find((t: Task) => t.id === id)!.subtasks?.length).toBe(0)
  })
  ...
  it('should add a reminder', () => {
    ...
    const r = useTaskStore.getState().reminders[useTaskStore.getState().reminders.length - 1]
    expect(r.isTriggered).toBe(false)
    expect(r.isRead).toBe(false)
  })

  it('should dismiss a reminder', () => {
    useTaskStore.getState().addReminder({ taskId: 't1', type: 'deadline', message: 'm', remindAt: Date.now() })
    const rId = useTaskStore.getState().reminders[0].id
    useTaskStore.getState().dismissReminder(rId)
    const dismissed = useTaskStore.getState().reminders.find((r) => r.id === rId)
    expect(dismissed?.isRead).toBe(true)
  })

  it('should batch update status', () => {
    const id1 = useTaskStore.getState().addTask({ title: 'A', description: '', status: 'todo', priority: 'low', type: 'bug', source: 'manual' })
    const id2 = useTaskStore.getState().addTask({ title: 'B', description: '', status: 'todo', priority: 'low', type: 'bug', source: 'manual' })
    useTaskStore.getState().batchUpdateStatus([id1, id2], 'done')
    expect(useTaskStore.getState().tasks.find((t: Task) => t.id === id1)!.status).toBe('done')
    expect(useTaskStore.getState().tasks.find((t: Task) => t.id === id2)!.status).toBe('done')
  })
  ...
  it('should NOT seed when tasks exist', () => {
    useTaskStore.getState().addTask({ title: 'Exists', description: '', status: 'todo', priority: 'low', type: 'other', source: 'manual' })
