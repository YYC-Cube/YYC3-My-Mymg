...
test.describe('Navigation — Page Switching', () => {
  ...
  test('can switch to settings page', async ({ page }) => {
    const settingsBtn = page.locator('button:has-text("设置"), [data-testid="nav-settings"]').first()
  ...
  test('can switch between core nav items', async ({ page }) => {
    ...
    if (count > 5) {
      await buttons.nth(5).click({ timeout: 3000 }).catch(() => {})
      await page.waitForTimeout(300)
      await expect(page.locator('#root')).toBeVisible()
      await buttons.nth(3).click({ timeout: 3000 }).catch(() => {})
      await page.waitForTimeout(300)
      await expect(page.locator('#root')).toBeVisible()
    }
  })
