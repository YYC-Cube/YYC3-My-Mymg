...
const DEV_WORKSPACE_NAV = 'Developer Workspace' // nav item text or PageId
...
test.describe('AI Assistant', () => {
  ...
  test('should send a message and receive mock response', async ({ page }) => {
    ...
    if (await input.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const response = page
        ...
        .first()
      const responseVisible = await response.isVisible({ timeout: 3000 }).catch(() => false)
  ...
  test('should show provider selector in config', async ({ page }) => {
    ...
    const visible = await providerSelect.isVisible({ timeout: 2000 }).catch(() => false)
...
test.describe('Panel Resizing', () => {
  ...
  test('should display panel width in status bar', async ({ page }) => {
    ...
    const visible = await widthIndicator.isVisible({ timeout: 3000 }).catch(() => false)
